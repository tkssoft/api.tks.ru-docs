...
### Алгоритм определения наличия преференций для РС (PRIZNAK=5)

* Если NOPREF == None: преф. режим для РС есть
* Иначе преф. режим для РС отсутствует
* Если NOPREF_PR == 1:
  * альтернативные варианты находятся в массиве TNVEDALL в признаке 5:
  * Если MIN == None: преф. режим отсутствует
  * Иначе преф. режим есть

Наличие преференция для НРС аналогичен, используются PREF92, PREF92_PR и сведения из TNVEDALL в признаке 32.
