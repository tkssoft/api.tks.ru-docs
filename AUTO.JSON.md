...
| `volume`       | Объём двигателя, см3                          | int    | [0-9]+                                                                             |
...
| `ts_type`      | Тип транспортного средства                    | str    | `00_8703` - автомобиль, `00_8704` - пикап, `01_8703` - квадроцикл, `02_8703` - гольф-кар, `03_8703101100` - снегоход, `04_8704` - грузовик, `05_870410` - самосвал, `06_8711` - мотоцикл, `07_8711201000` - мотороллер, `08_8711100000` - мопед, `09_8716400000` - автоприцеп, `10_871610` - дом-автоприцеп, `11_890399` - водный мотоцикл, `12_8903` - катер (яхта, лодка), `13_8702` - автобус |
| `sequential`    | Гибрид последовательного типа       | bool   | `true`, `false`                                                                    |
| `power_hybrid_dvs`| Мощность ДВС для параллельных гибридов                                     | int    | [0-9]+                                                                             |
| `power_hybrid_dvs_edizm`| Единицы измерения мощности ДВС для параллельных гибридов                   | str    | `ls` - лошадиные силы, `kvt` - кВт                                                 |
| `power_hybrid_electro`| Мощность ЭД для параллельных гибридов                                     | int    | [0-9]+                                                                             |
| `power_hybrid_electro_edizm`| Единицы измерения мощности ЭД для параллельных гибридов                   | str    | `ls` - лошадиные силы, `kvt` - кВт                                                 |
| `mass`         | Масса, т                                         | float    | [0-9\.,]+                                                                             |
...
| `movs_gt_m30ed`| Мощность ДВС больше максимальной 30-минутной мощности ЭД | bool | `true`, `false`                                                                    |
...
| `pp_152_minpromtorg_cost`| [ср. стоимость Минпромторга](https://www.tks.ru/db/avtoprice/), руб.      | int    | [0-9]+ |
...
В качестве результата API возвращает JSON-структуру, содержащую в себе информацию по пошлине, акцизу, таможенным платежам, утилизационному сбору и т.п. для указанных в запросе сведений, суммы этих значений а также курсы валют, использованные для расчета. Значения расчитаны исходя из курсов валют на текущую дату. В случае, если ЦБ опубликованы курсы на следующий день (как правило, после 17.00 по московскому времени), то используются эти значения.
